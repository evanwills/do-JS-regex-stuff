...
function completeUpdate (input, extraInputs, GETvars) {
  ...
  if (_extra === 'ACPP') {
    ...
    output = input.replace(acppchange, '<a href="./?a=2345726">$1</a>')
    return output
  }
  // else if(_extra === 'SUO'){
  ...
  // }
    ...
    output = removeAll(input)
    output = output.replace(_su, '<a class="js-remote-modal" href="./?a=2347176?unit=$2&amp;SQ_DESIGN_NAME=modal">$2</a>')
    return output
    // } else if (_extra === 'RAA'){
    ...
    var _raa = new RegExp('(<[^>]*>\\s*?Schedule.*?Offerings\\s*?<[^>]*>)(\\s*?<[^>]*>\\s*?Click.*?Descriptions\\s*?<[^>]*>|\\s*?<[^>]*>\\s*?<[^>]*>\\s*?Click.*?Descriptions\\s*?<[^>]*>\\s*?<[^>]*>)?', 'igm')
    output = input.replace(_raa, '$1<p><a href="./?a=2347163">Click here for Unit Descriptions</a></p>')
    return output
  } else if (_extra === 'SCM') {
    var SCMAL = new RegExp('(<a[^>]*>\\s*?)?([A-Z]{4}[0-9]{3})(</a>)?', 'gm')
    var SCMHB = new RegExp('<a[^>]*>(handbook)(</a>)', 'igm')
    output = removeAll(input)
    output = output.replace(SCMAL, '<a class="js-remote-modal" href="./?a=2347384?unit=$2&amp;SQ_DESIGN_NAME=modal">$2</a>')
    output = output.replace(SCMHB, '<a href="./?a=2345662">$1$2')
    return output
  } else if (_extra === 'U&C') {
    ...
    var findReplace = [
      ...
      { // two
        find: '(<td).*?(?:( rowspan="[0-9]")[^>]*(>)|( colspan="[0-9]")[^>]*(>)|(>))',
        replace: '$1$2$3$4$5$6'
      },
      { // three & six
        find: '</?p>',
      ...
      { // nine
        find: '(<t)d( colspan="[0-9]"| rowspan="[0-9]")(>)(.*?)(<\/t)d(>)',
        replace: '$1h$2$3$4$5h$6'
      },
      { // ten
        find: '(<tr>\\s*?<t[hd])(.*?<t[hd])(.*?<t[hd])(.*?<t[hd])(.*?<t[hd])(.*?<t[hd])(.*?<t[hd])(.*?<t[hd])(.*?<t[hd])(.*?<t[hd])(.*?</tr>)',
    ...
    return multiRegexReplace(removeAll(input), findReplace, 'igm')
  }
...
// START: Jon's Table Stripping
 ...
 var removeTable = new RegExp('(<table)[^>]*(>)', 'igm')
  ...
  var StripP = new RegExp('(<(t[dhr])>)<p>(.*?)<\/p>(</\\2>)', 'igm')
  var StripBR = new RegExp('(<t[dhr]>)(?:<br>|<\/br>)', 'igm')
  var _raat2 = new RegExp('(<table)[^>]*(>)', 'igm')
...
output = removeAll(input)
  ...
  output = output.replace(StripP, '$1$3$4')
  output = output.replace(StripBR, '$1')
  output = output.replace(_raat2, '$1$2')
...
// START: Jon's Schedule of Unit Offerings Tables
 ...
 var removeTable = new RegExp('(<table)[^>]*(>)', 'igm')
  ...
  var _raat2 = new RegExp('(<table)[^>]*(>)', 'igm')
	var _raatb = new RegExp('(<tbody)[^>]*(>)\\s*?<tr[^>]*>', 'igm')
	var _raatb2 = new RegExp('(<\/table>)', 'igm')
		var TDA = new RegExp('(<tr[^>]*>\\s*?<td)[^>]*(>.*?</td>\\s*?<td)[^>]*(>.*?</td>\\s*?<td)[^>]*(>.*?</td>\\s*?<td)[^>]*(>.*?</td>)', 'igm')
	var THA = new RegExp('(<tr class="nowrap"[^>]*>\\s*?<)td[^>]*(>.*?</)td(>\\s*?<)td[^>]*(>.*?</)td(>\\s*?<)td[^>]*(>.*?</)td(>\\s*?<)td[^>]*(>.*?</)td(>)', 'igm')


	 var rUL = new RegExp('<u>', 'igm')
 var rUL2 = new RegExp('<\/u>', 'igm')
 var rUL3 = new RegExp('<em>', 'igm')
 var rUL4 = new RegExp('<\/em>', 'igm')
...
output = removeAll(input)
  ...
  output = output.replace(StripBR, '$1')
  		output = output.replace(_raat2, '$1 class="table table-bordered table-hover table-striped"$2<tbody>')
	output = output.replace(_raatb, '$1$2<tr class="noWrap">')
	output = output.replace(_raatb2, '</tbody>$1')
		output = output.replace(TDA, '$1 headers="a1"$2 headers="a2"$3 headers="a3"$4 headers="a4"$5')
	output = output.replace(THA, '$1th id="a1"$2th$3th id="a2"$4th$5th id="a3"$6th$7th id="a4"$8th$9')
	  output = output.replace(rUL, '')
  output = output.replace(rUL2, '')
  output = output.replace(rUL3, '')
  output = output.replace(rUL4, '')
  return output
}
...
// START: Jon's remove underline
 ...
 function RUL (input) {
  ...
  // var content = ''
  // var tmp = ''
 var rUL = new RegExp('<ul>', 'igm')
 var rUL2 = new RegExp('<\/ul>', 'igm')
 var rUL3 = new RegExp('<em>', 'igm')
 var rUL4 = new RegExp('<\/em>', 'igm')
